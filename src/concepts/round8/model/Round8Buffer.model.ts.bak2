...
export const IncrementOperations = {
  /**
   * Increment buffer by 1 using Round8 carry logic
   */
  incrementRound8Buffer(buffer: Round8Buffer): void {
    let carry = 1; // Start with increment of 1

    // Start from rightmost rotation (rotation 0)
    for (let rotationNum = 0; rotationNum <= SINGLE_ROTATION_INDEX && carry !== 0; rotationNum++) {
      ...
      if (sum < 8) {
        ...
        // sum = 8, wrap to 1 and carry
        RotationOperations.setRotationValue(buffer, rotationNum, 1);
        carry = 1;
      }
    }
    ...
    if (carry !== 0) {
      throw new Round8Error(
        ...
        'Increment overflow: exceeded maximum positive value'
      );
    }
  },

  /**
   * Decrement buffer by 1 using Round8 borrow logic
   */
  decrementRound8Buffer(buffer: Round8Buffer): void {
    let borrow = 1; // Start with decrement of 1
    ...
    for (let rotationNum = 0; rotationNum <= SINGLE_ROTATION_INDEX && borrow !== 0; rotationNum++) {
      ...
      if (diff >= 1) {
        ...
        // diff = 0, wrap to 8 and borrow
        RotationOperations.setRotationValue(buffer, rotationNum, 8);
        borrow = 1;
      }
    }

    // If borrow still set after rotation 20, underflow
    if (borrow !== 0) {
      throw new Round8Error(
        Round8ErrorCode.OUT_OF_SAFE_INTEGER_RANGE,
        'Decrement underflow: exceeded minimum value'
      );
    }
  },
};
