...
export const IncrementOperations = {
  /**
   * Increment buffer by 1 using Round8 carry logic with mode handling
   *
   * Mode 0: Zero Origin → Positive Mode
   * Mode 1: Positive Trajectory → Continue Positive
   * Mode 2: Negative Trajectory → Move toward Zero
   */
  incrementRound8Buffer(buffer: Round8Buffer): void {
    // MODE 0: Zero Origin (Sign=0, all 000) → Enter Positive Mode
    if (SpecialPatterns.isBufferZero(buffer)) {
      RotationOperations.setSignBit(buffer, 1); // Positive trajectory
      RotationOperations.setRotationValue(buffer, 0, 0); // Position[0]=000 (displays as "1")
      return;
    }

    // MODE 2: Negative One (Sign=0, all 111) → Zero Origin
    ...
    // MODE 2: Negative trajectory → move toward zero (reduce magnitude)
    if (sign === 0) {
      let borrow = 1;
      for (let rotationNum = 0; rotationNum <= SINGLE_ROTATION_INDEX && borrow !== 0; rotationNum++) {
        const currentValue = RotationOperations.getRotationValue(buffer, rotationNum);
        const diff = currentValue - borrow;

        if (diff >= 0) {
          RotationOperations.setRotationValue(buffer, rotationNum, diff);
          borrow = 0;
        } else {
          // Wrap: 0 - 1 → 7, borrow from next
          RotationOperations.setRotationValue(buffer, rotationNum, 7);
          borrow = 1;
        }
      }

      // Check if we reached zero (all positions are 000)
      let allZero = true;
      for (let i = 0; i <= SINGLE_ROTATION_INDEX; i++) {
        if (RotationOperations.getRotationValue(buffer, i) !== 0) {
          allZero = false;
          break;
        }
      }
    ...
    // MODE 1: Positive trajectory → continue positive
    let carry = 1;
    for (let rotationNum = 0; rotationNum <= SINGLE_ROTATION_INDEX && carry !== 0; rotationNum++) {
      const currentValue = RotationOperations.getRotationValue(buffer, rotationNum);
      const sum = currentValue + carry;
      if (sum <= 7) {
        RotationOperations.setRotationValue(buffer, rotationNum, sum);
        carry = 0;
      } else {
        // sum = 8: wrap to 0 and carry
        RotationOperations.setRotationValue(buffer, rotationNum, 0);
        carry = 1;
      }
    }
    // If carry still set after rotation 20, overflow
    if (carry !== 0) {
      throw new Round8Error(Round8ErrorCode.OUT_OF_SAFE_INTEGER_RANGE, 'Increment overflow: exceeded maximum positive value');
    }
  },

  /**
   * Decrement buffer by 1 using Round8 borrow logic with mode handling
   *
   * Mode 0: Zero Origin → Negative Mode
   * Mode 1: Positive Trajectory → Move toward Zero
   * Mode 2: Negative Trajectory → Continue Negative
   */
  decrementRound8Buffer(buffer: Round8Buffer): void {
    // MODE 0: Zero Origin (Sign=0, all 000) → Enter Negative Mode (-1)
    if (SpecialPatterns.isBufferZero(buffer)) {
    ...
    // MODE 1: Positive trajectory → move toward zero (reduce magnitude)
    if (sign === 1) {
      let borrow = 1;
    ...
    // MODE 2: Negative trajectory → continue negative (increase magnitude)
    // Special case: if currently -1 (all 111), next is -2
    if (SpecialPatterns.isBufferNegativeOne(buffer)) {
      // -1 → -2: Set position[0]=001, others stay 000
      RotationOperations.setSignBit(buffer, 0); // Keep negative
      for (let i = 0; i <= SINGLE_ROTATION_INDEX; i++) {
        RotationOperations.setRotationValue(buffer, i, 0);
      }
      RotationOperations.setRotationValue(buffer, 0, 1); // Position[0]=001 (displays as "2", with sign makes "-2")
      return;
    }

    // Regular negative: increment magnitude
    let carry = 1;
    ...
    // Check if we reached all 111 (would be -1, but we're past it, so this is underflow)
    if (carry !== 0) {
      throw new Round8Error(Round8ErrorCode.OUT_OF_SAFE_INTEGER_RANGE, 'Decrement underflow: exceeded maximum negative value');
    }
  },
};
